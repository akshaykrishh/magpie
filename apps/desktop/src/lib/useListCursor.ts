...
// contenteditable element", see lib/keys.ts's `isTextEntryTarget`) --
// applied here too, one task earlier, because arrow keys need it just as
// much: without it, ArrowUp/ArrowDown pressed while editing a multi-line
// capture body (a <textarea> nested inside the cursor-scoped list
// container) would have their native caret-movement default action
// canceled by this ancestor handler, breaking in-place multi-line editing
// to move an invisible list cursor instead.
